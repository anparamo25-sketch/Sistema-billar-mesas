...
if start >= 0 and end > start:
    method = r'''  String _tvHtml() => r''' + "'''" + r'''<!doctype html>
...
function elapsed(a){if(!a)return'00:00:00';let x=Math.max(0,Date.now()-new Date(a).getTime())/1000,h=Math.floor(x/3600),m=Math.floor(x%3600/60),s=Math.floor(x%60);return String(h).padStart(2,'0')+':'+String(m).padStart(2,'0')+':'+String(s).padStart(2,'0')}
function render(tables){grid.innerHTML=tables.map(g=>{let c=g.active?'playing':(g.finishedAt&&!g.paid?'pending':'');let state=g.active?'OCUPADA':(g.finishedAt&&!g.paid?'PENDIENTE DE COBRO':'DISPONIBLE');let tm=g.active?elapsed(g.startedAt):elapsed(g.startedAt,g.finishedAt);return '<section class="card '+c+'"><div class="name">Mesa '+g.tableId+'</div><div class="state">'+state+'</div><div class="time">'+tm+'</div><div class="info">Inicio: '+(g.startedAt?new Date(g.startedAt).toLocaleTimeString('es-NI',{hour:'2-digit',minute:'2-digit',hour12:true}):'--')+'</div><div class="info">Tarifa: '+money(g.rate)+'/hora</div><div class="total">Total: '+money(g.total)+'</div></section>'}).join('')}
