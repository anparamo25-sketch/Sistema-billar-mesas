...
function connect(){try{let ws=new WebSocket("ws://"+location.host);ws.onmessage=e=>{try{update(JSON.parse(e.data))}catch(_){}};ws.onclose=()=>setTimeout(connect,2000)}catch(_){setTimeout(connect,2000)}}
</script>
</body>
</html>'''
    ...
    p.write_text(s)
    print('Parche TV aplicado: HTML generado como string Dart normal, sin triple comillas.')
else:
