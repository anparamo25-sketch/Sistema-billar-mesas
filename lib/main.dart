...
const port = 8080;
const currentBuild = 122;
const currentVersion = '1.2.2';
const updateManifestUrl = 'https://raw.githubusercontent.com/anparamo25-sketch/Sistema-billar-mesas/main/update.json';
...
class _CentralState extends State<Central> with WidgetsBindingObserver {
  ...
  Future<void> checkForUpdates() async {
    ...
    try {
      ...
      final request = await client.getUrl(Uri.parse(updateManifestUrl));
      ...
      final remoteBuild = _buildFromVersion(remoteVersion);
      final download = data['downloadUrl']?.toString() ?? '';

      if (!mounted) return;
      Navigator.of(context).pop();

      if (remoteBuild <= currentBuild || download.isEmpty) {
        await showDialog<void>(
          context: context,
          builder: (_) => AlertDialog(
            title: const Text('Sin actualizaciones'),
            content: Text('La aplicación ya está actualizada. Versión $currentVersion (build $currentBuild).'),
            actions: [
              FilledButton(
      ...
      final install = await showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text('Hay una nueva versión $remoteVersion disponible.'),
          actions: [
            TextButton(
      ...
      await showDialog<void>(
        context: context,
        builder: (_) => AlertDialog(
          title: const Text('No se pudo comprobar'),
          content: const Text('No fue posible consultar el servidor de actualizaciones. Comprueba la conexión a Internet e inténtalo de nuevo.'),
          actions: [
            FilledButton(
              onPressed: () => Navigator.pop(context),
              child: const Text('Aceptar'),
  ...
  int _buildFromVersion(String version) {
    ...
    if (plus >= 0) return int.tryParse(version.substring(plus + 1)) ?? 0;
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (_, i) {
                            ...
                            return ListTile(
                              ...
                              title: Text('Mesa ${g.id} • ${money(g.total)}'),
                              subtitle: Text(
